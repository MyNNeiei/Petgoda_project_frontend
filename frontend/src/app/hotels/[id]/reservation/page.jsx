...
const NewReservation = () => {
    ...
    const { id: hotelId } = useParams();  // ✅ ดึงค่า hotelId จาก URL
    ...
    useEffect(() => {
        ...
        const fetchRooms = async () => {
            ...
            try {
                ...
                const availableRooms = response.data.filter(room => room.current_pets_count_int < room.max_pets);
                setRooms(availableRooms);

                // ✅ ถ้าไม่มีห้องว่างเลย ให้แจ้งเตือน และ reset โรงแรมที่เลือก
                if (availableRooms.length === 0) {
                    toast({
                        title: "No Available Rooms 🚫",
                        description: "This hotel has no available rooms for the selected dates.",
                        variant: "destructive",
                    });

                    setFormData(prev => ({ ...prev, hotel: "", room: "" }));
                    setHotels([]); // ล้างรายการโรงแรมเพื่อบังคับให้เลือกใหม่
                }
            } catch (error) {
    ...
    // ฟังก์ชันเช็คห้องที่ว่างในช่วงวันที่เลือก
const fetchRooms = async () => {
    setFetchingRooms(true);
    try {
        const response = await axiosInstance.get(`/api/hotels/${formData.hotel}/rooms/`, {
        ...
        const availableRooms = response.data.filter(room => {
            // เช็คว่าห้องนี้ว่างสำหรับช่วงวันที่เลือก
            const isBooked = room.reservations.some(reservation => {
                ...
                return (
                    // เช็คว่ามีการทับซ้อนของวันที่หรือไม่
                    (isBefore(reservationEnd, formData.checkInDate) || isBefore(formData.checkOutDate, reservationStart))
                );
            });

            // ถ้าไม่มีการจองซ้ำ จึงจะถือว่าว่าง
            return !isBooked;
        });
        ...
        try {
            ...
            if (!response.data.available) {
                toast({
                    ...
                    description: "This room is fully booked for the selected dates. Please choose another room.",
                    variant: "destructive",
                });

                // ✅ รีเซ็ตค่า room เพื่อบังคับให้เลือกใหม่
                setFormData((prev) => ({ ...prev, room: "" }));
            }
        } catch (error) {
            console.error("❌ Error checking room availability:", error);
            toast({ title: "Error", description: "Failed to check room availability.", variant: "destructive" });
        }
    };
    ...
    const handleSubmit = async () => {
        if (!formData.hotel || !formData.room || !formData.checkInDate || !formData.checkOutDate || !formData.pet) {
        ...
        try {
            ...
            // ✅ ดึง user_id จาก token หรือ API endpoint ที่เกี่ยวข้อง
            ...
            const user_id = userResponse.data.id; // ✅ ใช้ user_id ของผู้ที่ล็อกอินอยู่

            // ส่งข้อมูลไปยัง API Django
            await axiosInstance.post("/api/reservations/create/", {
                user_id,  // ✅ ใช้ user_id ที่ได้จาก API
                pet_id: formData.pet, // ✅ ผู้ใช้เลือกสัตว์เลี้ยงที่ต้องการจอง
